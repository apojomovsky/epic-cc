...
| **P5** | **DONE** Interrupts: single-vector compatibility mode (IPEN=0), one handler at `0x0008` (see the P5 note and ADR-011). | `interrupt_pic18.c`, `interrupt_gate_pic18.c`; `interrupt_mul.c` joins P6 (it needs `*`/`/`) |
...
[ADR-011](adr/ADR-011-pic18-interrupts.md): v1 targets the single-vector
...
>>>>>>> f90fc5b (docs: ADR-011 PIC18 interrupts and P5 DONE mark)
