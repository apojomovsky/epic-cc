...
fn all_contains_every_seed_device() {
    // ALL is generated by scanning every file under devices/, so the registry must
    // mirror the shipped TOMLs exactly: a count pin here would break on
    // every onboarding, while a set comparison catches a dropped or
    // phantom device for good.
    let manifest = std::env::var("CARGO_MANIFEST_DIR").unwrap();
    let devices_dir = std::path::Path::new(&manifest).join("devices");
    let mut shipped: Vec<String> = std::fs::read_dir(devices_dir)
        .unwrap()
        .map(|e| e.unwrap().path())
        ...
        .collect();
    shipped.sort();
    let mut registered: Vec<String> = device::ALL.iter().map(|d| d.name.to_string()).collect();
    registered.sort();
    assert_eq!(
        registered, shipped,
        "ALL must mirror the TOMLs under crates/device/devices/"
    );
...
#[test]
fn config_names_resolve_unambiguously() {
    // resolve_config takes the first field (and value) whose name or alias
    // matches, so a spelling shared by two of them would resolve by order.
    for d in device::ALL {
        let mut fields = std::collections::HashSet::new();
        for f in d.config.fields {
            for n in std::iter::once(&f.name).chain(f.aliases) {
                assert!(
                    fields.insert(n.to_ascii_lowercase()),
                    "{}: config name {n} is ambiguous",
                    d.name
                );
            }
            ...
            for v in f.values {
                for n in std::iter::once(&v.name).chain(v.aliases) {
                    assert!(
                        values.insert(n.to_ascii_lowercase()),
                        "{}: {}={n} is ambiguous",
                        d.name,
                        f.name
                    );
