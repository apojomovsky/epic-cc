...
fn probe_edge_cases() {
    ...
    fn f32_le(x: f32) -> [u8; 4] { x.to_bits().to_le_bytes() }
    fn sim_run_bytes(ir_text: &str, map: &[(String, u16)], seed: &[(u16, u8)], out: u16, n: usize) -> Vec<u8> {
        ...
        for (a, v) in seed { p.ram_mut()[*a as usize] = *v; }
        p.run(200_000);
    ...
    fn float_routine_module(name: &str) -> (String, Vec<(String, u16)>) {
        ...
        let mut map = vec![
            ("ina".to_string(), 0x20u16), ("inb".to_string(), 0x24), ("out".to_string(), 0x28),
            ("main::x".to_string(), 0x2C), ("main::y".to_string(), 0x30), ("main::r".to_string(), 0x34),
        ];
        let mut base = 0x40u16;
        for (pn, _) in params { map.push((format!("{name}::{pn}"), base)); base += 4; }
    ...
    let cases: &[(&str, f32, f32)] = &[
        ...
        ("__add_f32", f32::from_bits(0x00000001), f32::from_bits(0x00000001)), // denormal+denormal
    ...
    for &(name, a, b) in cases {
        ...
        for (i, by) in f32_le(a).iter().enumerate() { seed.push((0x20 + i as u16, *by)); }
        for (i, by) in f32_le(b).iter().enumerate() { seed.push((0x24 + i as u16, *by)); }
        let want = match name {
        ...
        eprintln!("{name}({a:?}, {b:?}): got {got:02X?} ({got_f:?}) want {want:02X?} ({want_f:?}) {}", if ok { "OK" } else { "MISMATCH" });
    }
}
