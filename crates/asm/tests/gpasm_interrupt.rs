...
//! (`MOVWF 0x75` / `SWAPF STATUS, W` / `MOVWF 0x76` / PCLATH/FSR ->
//! 0x77/0x78 / PCLATH = 0), the ISR body (a direct SFR store `MOVWF 0x06`
//! from the `inttoptr` literal pointer, a call to the duplicated shared
//! helper `bump_isr`), the restore epilogue (`SWAPF 0x76, W` back into
//! STATUS, W last) and `RETFIE` — the SWAPF/RETFIE encodings gpasm verifies.
//!
...
//! interrupt_e2e.rs): fire the interrupt at main's word 58 (the `%2 = load
...
fn interrupt_hex_matches_gpasm_and_runs() {
    ...
    // word 58 -> out = 0x16, PORTB = 0x22, halted.
