...
size-report: image ## Regenerate crates/driver/tests/fixtures/SIZE_REPORT.md from the tree
	...
	@note=$$(python3 scripts/size-refresh-xc8.py --snapshot crates/driver/tests/fixtures/xc8_snapshot.json --out crates/driver/tests/fixtures/xc8_snapshot.json --bench-dir crates/driver/tests/fixtures/size-bench --repo $(CURDIR)); \
	python3 scripts/size-report.py --sizes scratch/size-report/sizes.json --density scratch/size-report/density.json --baseline crates/driver/tests/fixtures/size_baseline.toml --xc8 crates/driver/tests/fixtures/xc8_snapshot.json --sha $(EPIC_CC_GIT_SHA) --date $(REPORT_DATE) --oracle "$$note" --asm-inputs scratch/size-report/asm-inputs.txt --out crates/driver/tests/fixtures/SIZE_REPORT.md && echo "wrote crates/driver/tests/fixtures/SIZE_REPORT.md ($$note)"
