...
pub struct CallGraph {
    ...
    pub max_depth: usize,
    /// The call site of each edge's first reference, when the module
    /// carries debug locations. Recursion diagnostics name it.
    pub edge_locs: HashMap<(String, String), SrcLoc>,
}
...
pub fn build(m: &Module) -> CallGraph {
    ...
    let mut edge_locs: HashMap<(String, String), SrcLoc> = HashMap::new();
    for f in &m.funcs {
                ...
                if let Inst::Call(c) = inst {
                    ...
                    if c.callees.is_empty() {
                        ...
                        for callee in &c.callees {
                            ...
                            if let Some(loc) = &c.loc {
                                edge_locs
                                    .entry((f.name.clone(), callee.clone()))
                                    .or_insert(loc.clone());
                            }
                        }
                    }
    ...
    for f in &m.funcs {
        max_depth = max_depth.max(dfs_depth(&f.name, &adj, &mut color, &mut memo, &edge_locs));
    }
...
fn dfs_depth(
    ...
    edge_locs: &HashMap<(String, String), SrcLoc>,
) -> usize {
    match color.get(node).copied().unwrap_or(0) {
        2 => return memo[node],
        // Only the callee loop below can reach a gray node, and it reports
        // the back edge with its call site; this arm stays as a guard.
    ...
    for callee in adj.get(node).map(Vec::as_slice).unwrap_or(&[]) {
        if color.get(callee).copied() == Some(1) {
            match edge_locs.get(&(node.to_string(), callee.to_string())) {
                Some(loc) => {
                    panic!("{loc}: callgraph: recursion detected (call cycle involving {callee})")
                }
                None => panic!("callgraph: recursion detected (call cycle involving {callee})"),
            }
        }
        deepest = deepest.max(1 + dfs_depth(callee, adj, color, memo, edge_locs));
    }
