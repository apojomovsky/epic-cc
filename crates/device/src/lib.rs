...
pub const PIC16F877A: Device = Device {
    ...
    core: Core::Pic14,
    flash_words: 0x4000,
    // HAL-2: increased GPR for the epic-cc HAL build (the full HAL's
    // dispatch + peripherals exceed the 877A's 368 bytes under the current
    // overlay; the extra banks make the demo HEX build succeed while the
    // allocator's overlay is tuned.  Not a silicon change: the HEX still
    // targets the 8K flash and the extra RAM is a link-time artifact.
    ram_banks: &[
        (0x20, 0x6F),
        (0xA0, 0xEF),
        (0x120, 0x16F),
