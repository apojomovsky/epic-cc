...
// with a shift-add chain instead of unrolled per-byte adds, on both
// scaled pointer paths: FSR0/INDF0 for the volatile RAM array and
// TBLPTR/TBLRD for the flash const array. A wrong chain lands every
// access on the wrong element, so the sim assertion catches it.
//
