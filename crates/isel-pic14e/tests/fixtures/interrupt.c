// P5 (PIC14E) interrupt acceptance: SFR access via inttoptr, a noinline
...
// F877A bank-mirrored SFR) to 0x0D (the 1937's PORTB, gpasm header line:
// `PORTB EQU H'000D'`). Everything else is byte-identical: the INTCON bit
// game stays out of this fixture (the e2e fires the interrupt directly via
// Pic14e::fire_interrupt, exactly like the PIC14 interrupt_e2e test).
//
// `in` and `out` are volatile globals (their addresses come from the alloc
// layout the driver used); PORTB is the 1937 SFR at absolute address 0x0D
// (`*(volatile unsigned char *)0x0D` -> clang's `inttoptr (i16 13 to ptr)`).
//
...
//
// The e2e fires the interrupt at the traced pc (right after the
// `PORTB = 0x11` store), the ISR preempts main before the shared helper's
// argument is read. Hand computation (in = 0x10):
//   main: out = in                        -> 0x10
//   main: PORTB = 0x11
//   <- ISR fires here
...
#define PORTB (*(volatile unsigned char *)0x0D) // SFR access via inttoptr
