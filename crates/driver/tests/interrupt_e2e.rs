...
//!
//! The injection point is main's **word 74** (`%2 = load out`, the argument
//! load of `out = bump(out)`, immediately after the `PORTB = 0x11` store at
//! word 73), verified against the exact emitted asm (crates/asm/tests/
...
/// The interrupt vector (word 4) and the injection point (word 74) as
/// documented above.
const VECTOR: u16 = 4;
const INJECT_PC: u16 = 74;
...
fn interrupt_runs_correctly_with_mid_run_fire() {
    ...
    // Run main to the injection point (word 77): the `%2 = load out` for
