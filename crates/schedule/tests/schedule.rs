...
fn sinks_a_w_and_flag_free_excursion_past_its_successor() {
    ...
    assert_eq!(
        order,
        vec!["    MOVWF 0x20", "    MOVWF 0x21", "    BSF 0xA5, 3"],
...
fn falls_back_to_hoisting_when_the_successor_is_a_skip_op() {
    ...
    assert_eq!(
        order,
        vec!["    BSF 0xA5, 3", "    MOVWF 0x20", "    BTFSC 0x21, 2"],
...
/// The corrected `EPIC_IRQ_Enable` shape: `cur` (`MOVWF 0xA5`) reads W,
...
fn phase2_declines_a_skip_target_excursion_instruction() {
    // cur (MOVWF 0xA5) immediately follows a skip op (BTFSC 0x22, 2,
...
#[test]
fn sfr_accesses_split_regions() {
    // PIC16F877A EEPROM window (DS39582C): EEDATA 0x10C and EEADR 0x10D
    // in bank 2 are SFRs, 0x20 is a GPR. SFR touches end the region so
    // no motion can cross them.
    let asm = "    MOVWF 0x18C\n    MOVF 0x10C, W\n    MOVWF 0x20\n";
    let lines = classify(&PIC16F877A, asm);
    assert!(matches!(&lines[0], Line::Insn(i) if i.touches_sfr));
    assert!(matches!(&lines[1], Line::Insn(i) if i.touches_sfr));
    assert!(matches!(&lines[2], Line::Insn(i) if !i.touches_sfr));
    assert_eq!(regions(&lines), vec![2..3]);
}

#[test]
fn schedule_never_hoists_sfr_stores_across_sfr_loads() {
    // The phase-2 excursion shape from the data-EEPROM probe
    // (epic-cc#345): without the barrier, the EEADR store bundle
    // (`MOVLW 0x11`/`MOVWF 0x10D`) hoists above the EEDATA load
    // (`MOVF 0x10C, W`), sinking the first read below the second RD.
    let asm = "    MOVWF 0x18C\n    MOVWF 0x10C\n    MOVF 0x10C, W\n    MOVWF 0x20\n    MOVLW 0x11\n    MOVWF 0x10D\n";
    assert_eq!(schedule(&PIC16F877A, asm), asm);
}
