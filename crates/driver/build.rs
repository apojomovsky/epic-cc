...
fn emit_rerun_hints() {
    ...
    if let Some(ref_name) = git(&root, &["rev-parse", "--symbolic-full-name", "HEAD"]) {
        if let Some(path) = git(&root, &["rev-parse", "--git-path", &ref_name]) {
            ...
            // (`.git/refs/heads/main`); in a worktree it is already absolute
            // (the ref lives in the common dir). Resolve relative output
            // against the root git was run in, or the existence check would
            // test it under the package root and drop a path that is there.
