// Milestone-13 interrupt acceptance: SFR access via inttoptr, a noinline
...
// layout the driver used); PORTB is the F877A SFR at absolute address 0x06
// (`*(volatile unsigned char *)0x06` -> clang's `inttoptr (i16 6 to ptr)`).
//
...
//
// The e2e fires the interrupt at main's word 75 (`%2 = load out` for
// `out = bump(out)`, right after the `PORTB = 0x11` store at word 74
...
//   <- ISR fires here (pc == 75; W = 0x11, out = 0x10)
...
// (see crates/driver/tests/interrupt_e2e.rs).
