...
//!
//! The injection point is main's **word 58** (`%2 = load out`, the argument
//! load of `out = bump(out)`, immediately after the `PORTB = 0x11` store at
//! word 57) — verified against the exact emitted asm (crates/asm/tests/
...
//!   main: out = in                          -> 0x10   (word 55 store)
//!   main: PORTB = 0x11                      (word 57 store)
//!   <- fire_interrupt at pc == 58: push 59, jump to the vector (word 4)
//!   isr:  save W/STATUS/PCLATH/FSR -> 0x75-0x78
//!         PORTB = 0x55                      (SFR write from the ISR)
//!         out = bump_isr(out = 0x10)        -> 0x11   (the _isr duplicate)
//!         restore; RETFIE -> pc == 59
...
/// The interrupt vector (word 4) and the injection point (word 58) as
/// documented above.
const VECTOR: u16 = 4;
const INJECT_PC: u16 = 58;
...
fn interrupt_runs_correctly_with_mid_run_fire() {
    ...
    // Run main to the injection point (word 58): the `%2 = load out` for
    ...
    // Fire the interrupt: push pc+1 (59), jump to the vector at word 4.
    p.fire_interrupt();
    assert_eq!(p.pc(), VECTOR, "the ISR starts at the vector (word 4)");

    // The ISR runs (PORTB = 0x55, out = bump_isr(out)), RETFIE returns to
    // word 59, and main completes: out == 0x16, PORTB == 0x22, then the
