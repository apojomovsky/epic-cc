...
// rewrites main's store to the copy; the e2e fires the interrupt mid-run and
// checks the ISR's callback ran and wrote the expected value.
//
// Hand computation:
...
fn run_one(device_name: &str, device: &device::Device) {
    ...
    match device.core {
        device::Core::Pic14 => {
            ...
            // callback store precedes it), then fire the interrupt.
            ...
            // The ISR runs (g_cb() -> on_event_isr: out = 0x55), RETFIE
            // returns to main, and __start SLEEP halts the machine.
            p.run(500_000);
            assert_eq!(
                p.ram()[out_addr],
                0x55,
        ...
        device::Core::Pic18 => {
            ...
            // Run main to the point right after the `out = 0x11` store, then
            // fire the interrupt (high vector 0x0008).
            let mut steps = 0usize;
            while p.ram()[out_addr] != 0x11 {
                p.step();
                steps += 1;
