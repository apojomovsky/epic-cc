...
- [ ] **Step 2: Run to verify it fails**.
- [ ] **Step 3: Implement — REFACTOR to a single shared slot map first.** The milestone-1 isel allocates slots per function, each starting at `COMMON_START` (0x70) — with calls, the callee's param slots collide with the caller's live slots and the arg-copy clobbers the caller's values. **Must refactor to the spike's structure (spike/src/codegen.rs): one module-wide `ssa` map keyed `{func}::{name}` with a single shared allocator counter** across all functions (common RAM 0x70–0x7F first, then bank-0 GPR from `bank0_start`). Compute `end_of_globals` from the address map (max global addr + size), then `scratch = end_of_globals`, `retval_lo = end+1`, `retval_hi = end+2`, `bank0_start = end+3` (probe: globals 0x20/0x21 → scratch 0x22, retval 0x23/0x24, bank0 0x25). Keep the icmp-scratch reservation (Task 6 fix) keyed to the new `scratch` value. Then `Call` copies args into the callee's `{callee}::{param}` slots (natural in the shared map), `CALL`, and copies retval into `dst`; `Ret(Some(...))` copies into retval slots + `RETURN`. Existing isel tests must still pass (they use single-function modules; the refactor changes nothing observable there except keying).
- [ ] **Step 4: Run to verify it passes**.
