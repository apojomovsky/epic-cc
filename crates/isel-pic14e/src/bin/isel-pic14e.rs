use isel_pic14e::{parse_map, select};
use std::fs;

/// `isel-pic14e <in.ir> <in.map> <out.asm>`
///
...
/// `{func}::{name}`, matching the keys the backend looks up.
fn main() {
    ...
    let asm = select(&device::PIC16F1937, &ir::parse(&src), &addrs);
