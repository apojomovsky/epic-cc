...
#[test]
fn pic18_float_hex_matches_gpasm() {
    let src = include_str!("fixtures/pic18_float.asm");
    let ours = assemble_file_to_hex(&device::PIC18F4550, src);
    let dir = concat!(env!("CARGO_MANIFEST_DIR"), "/tests/fixtures");
    std::fs::write(format!("{dir}/pic18_float_ours.hex"), &ours).unwrap();
    let gpasm_src = to_gpasm_src(src);
    let gpasm_asm = std::env::temp_dir().join("pic18_float_gpasm.asm");
    std::fs::write(&gpasm_asm, &gpasm_src).unwrap();
    let out = Command::new(gpasm())
        .args(["-p", "p18f4550", gpasm_asm.to_str().unwrap(), "-o", "/tmp/pic18_float_gpasm.hex"])
        .output()
        .expect("run gpasm");
    assert!(out.status.success(), "gpasm: {}", String::from_utf8_lossy(&out.stderr));
    let theirs = std::fs::read_to_string("/tmp/pic18_float_gpasm.hex").unwrap();
